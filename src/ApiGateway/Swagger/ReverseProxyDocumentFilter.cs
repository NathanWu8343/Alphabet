...
namespace ApiGateway.Swagger
{
    public sealed class ReverseProxyDocumentFilter : IDocumentFilter
    {
                    ...
                    foreach (var swagger in destination.Value.Swaggers)
                    {
                        ...
                        foreach (var swaggerPath in swagger.Paths)
                        {
                            ...
                            foreach (var path in doc.Paths)
                            {
                                ...
                                {
                                    var pathKey = GetNewApiPath(swagger.PrefixPath, swagger.PathRemovePrefix, path.Key);
                                ...
                                paths.TryAdd(GetNewApiPath(swagger.PrefixPath, swagger.PathRemovePrefix, key), value);
                            }
        ...
        private static string GetNewApiPath(string prefixPath, string pathRemovePrefix, string apiPath)
        {
            return string.IsNullOrEmpty(pathRemovePrefix)
                ? $"{prefixPath}{apiPath}"
                : $"{prefixPath}{apiPath.Replace(pathRemovePrefix, "")}"; //TODO: 待優話來保證前面自斷移除
        }
    }
}